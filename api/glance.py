...
log.info("Glance API Validation - Begin")
...
if token is not None:
    headers = {'Content-Type' : 'application/json',
               'X-Auth-Token' : token}
    endpoint = keystone_client.get_public_endpoint('glance')
    url = endpoint + '/v2/images'
    response = request.get(url, headers=headers)
    if response.status_code != 201:
        log.info('Glance call failed with error code %s ' % response.status_code)
    else:
        log.info('Successfully retrieved server details for %s ' % tenant_id)
else:
    log.info('Auth token null. Skipping Glance calls')
log.info("Glance API Validation - End")